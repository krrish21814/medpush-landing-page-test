  ...
  return (
    <>
      ...
      <div
        ...
        } top-0 left-0 right-0 z-50 bg-[#FFF0F1] px-4 md:px-0 transition-all duration-300 ${
          isScrolled ? 'py-2 md:py-3 shadow-xl' : 'py-3 md:py-4'
        }`}
      >
        {/* White Rounded Card */}
        <header
          className={`max-w-[1400px] mx-auto bg-white rounded-full shadow-lg px-4 md:px-8 transition-all duration-300 ${
        ...
        >
          <div className="flex items-center justify-between relative">
                ...
                link.hasDropdown ? (
                  <div key={link.name} className="relative" ref={dropdownRef}>
                    <button
                      ...
                      className={`text-gray-900 hover:text-[#C11E2B] transition-colors duration-200 text-sm xl:text-base font-medium flex items-center gap-1 pb-1 ${
                  ...
                  <Link
                    key={link.name}
                    href={link.href}
                    className={`text-gray-900 hover:text-[#C11E2B] transition-colors duration-200 text-sm xl:text-base font-medium pb-1 ${
                      pathname === link.href
                        ? 'text-[#C11E2B] border-b-2 border-[#C11E2B]'
                        : ''
                    }`}
            ...
            <div className="hidden lg:flex items-center gap-2 xl:gap-4">
              ...
              <Link
                href="/contact-us"
                className={`px-4 xl:px-8 transition-all duration-300 bg-[#C11E2B] text-white rounded-full font-semibold hover:bg-[#a01822] transition-colors duration-200 flex items-center gap-2 shadow-md text-sm xl:text-base ${
        ...
        {isMenuOpen && (
          <div className="lg:hidden bg-white rounded-3xl mt-4 mx-4 shadow-xl p-6">
            {navLinks.map((link) =>
              link.hasDropdown ? (
                <div key={link.name}>
                  <button
                    onClick={() => setIsOfferingsOpen(!isOfferingsOpen)}
                    className="w-full text-left py-3 text-gray-900 hover:text-[#C11E2B] transition-colors duration-200 font-medium text-base flex items-center justify-between"
                  >
                    {link.name}
                    <ChevronDown
                      className={`w-4 h-4 transition-transform duration-200 ${
                    ...
                    <div className="pl-4 space-y-2">
                      {offeringsDropdown.map((item) => (
                        <Link
                          key={item.name}
                          href={item.href}
                          className="block py-2 text-sm text-gray-600 hover:text-[#C11E2B]"
                          onClick={() => {
                            setIsMenuOpen(false);
                            setIsOfferingsOpen(false);
                          }}
            ...
            <Link 
              href="/contact-us"
              className="block w-full mt-4 px-6 py-3 bg-[#C11E2B] text-white rounded-full font-semibold text-center"
              onClick={() => setIsMenuOpen(false)}
            >
              Contact Us
            </Link>
